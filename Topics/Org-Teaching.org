...
As explained earlier, you can update the class schedule in the syllabus with the new lecture information.
...
As you can see, class materials that have the =slideonly= (=handoutonly=) tag are not
exported in the LaTeX (reveal.js) output. The tags allow you to specify any content you
